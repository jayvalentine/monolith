...
},{"process":"../node_modules/process/browser.js"}],"random.ts":[function(require,module,exports) {
"use strict";
...
},{"./tribe":"tribe.ts","./random":"random.ts","./language":"language.ts","./tribe_events_disasters":"tribe_events_disasters.ts","./tribe_events_discovery":"tribe_events_discovery.ts"}],"region_events.ts":[function(require,module,exports) {
"use strict";
...
function () {
  ...
  LandingEvent.outcomeFunction = function (region) {
    return function () {
      console.log("Landing event triggered in " + region.typeDescription() + " region.");
    };
  };
...
exports.RegionEvents = [LandingEvent];
...
var region_events_1 = require("./region_events");

var tribe_1 = require("./tribe");

var tribe_events_1 = require("./tribe_events");
...
function () {
  ...
  Game.prototype.start = function () {
    ...
    this.choices = [];
    this.outcomes = [];
    this.outcomeMessages = []; // Generate a number of regions.

    var regionLimit = random_1.Random.interval(30, 50);

    for (var i = 0; i < regionLimit; i++) {
      // Create a new region.
      var r = new region_1.Region(); // Add as many tribes as there are food in the region.

      for (var f = 0; f < r.food(); f++) {
        var t = new tribe_1.Tribe(random_1.Random.interval(30, 90));
        r.addTribe(t);
      }
    ...
    var _loop_1 = function _loop_1(region) {
      // Get the list of other regions (i.e. filter this one out of the full list).
      var otherRegions = this_1.regions.filter(function (value, index, array) {
        return value != region;
      }); // 1-4 neighbours for each region.

      var numNearby = random_1.Random.interval(1, 4);

      var _loop_4 = function _loop_4(i) {
        // Pick another region from the list.
        var other = random_1.Random.choice(otherRegions); // Set these two regions as neighbours.

        region.addNearbyRegion(other); // Exclude the region we've just picked from further selection.

        otherRegions = otherRegions.filter(function (value, index, array) {
          return value != other;
        });
      };
    ...
    var this_1 = this; // Generate a number of neighbours for each region.
    ...
    this.queueMessage("You awaken in a cold void. A glistening blue-green marble hangs below you.\n      You slowly become aware of the various sensors attached to you, reading\n      magnetic fields, temperature, radiation, and more.\n      You observe Kepler 62-f slowly, and search for a landing site.", function () {}); // Determine a number of landing sites.

    var landingSites = [];
    var regionsCopy = this.regions; // Determine number of sites to pick.

    var landingSitesLimit = random_1.Random.interval(4, 8);

    var _loop_2 = function _loop_2(i) {
      var r = random_1.Random.choice(regionsCopy);
      landingSites.push(r);
      regionsCopy = regionsCopy.filter(function (value, index, array) {
        return value != r;
      });
    ...
    var _loop_3 = function _loop_3(r) {
      ...
      landingSiteOutcomes.push(function () {
        r.hasMonolith = true;
      });
  ...
  Game.prototype.run = function () {
    ...
    if (this.choiceFlag == Game.Choice.Started) {
      ...
      var tribesCount = 0; // See if any region events trigger.

      for (var _i = 0, _a = this.regions; _i < _a.length; _i++) {
        var region = _a[_i];

        for (var _b = 0, RegionEvents_1 = region_events_1.RegionEvents; _b < RegionEvents_1.length; _b++) {
          var regionEvent = RegionEvents_1[_b]; // If this region event triggers on this region, queue a message.
        ...
        for (var _c = 0, _d = region.tribes(); _c < _d.length; _c++) {
          var tribe = _d[_c];
          if (tribe.dead) tribesToRemove.push(tribe);else {
            ...
            for (var _e = 0, TribeEvents_1 = tribe_events_1.TribeEvents; _e < TribeEvents_1.length; _e++) {
              var tribeEvent = TribeEvents_1[_e]; // If this tribe event triggers on this tribe, queue a message or a choice.
      ...
      this.day += 1; // Trigger again in 50ms.
      ...
      jquery_1.default("#GameDate").text("Year " + Math.floor(this.day / 276) + ", Day " + this.day % 276 + ". Population " + populationString + ", " + tribesCount + " Tribes (Encountered).");
    }
  };
  ...
  Game.prototype.makeChoice = function (choice) {
    ...
    if (this.choiceFlag != Game.Choice.Waiting) return; // Remove all choices except the one selected.

    for (var i = 0; i < this.choiceIndex; i++) {
      if (i != choice) {
        console.log("Deleting choice " + i + " from the DOM.");
        jquery_1.default("#choice-" + i).remove();
      } else {
        jquery_1.default("#choice-" + i).removeClass('clickable');
        jquery_1.default("#choice-" + i).addClass('choice');
        jquery_1.default("#choice-" + i).removeAttr('id');
      }
    }
  ...
  Game.prototype.displayMessage = function (message) {
    ...
    jquery_1.default("<p>" + message + "</p>").insertBefore("#GameDate").hide().fadeIn(1000); // Trigger the run method 2 seconds from now.

    setTimeout(this.run.bind(this), 2000); // Force scroll to the bottom of the screen.

    jquery_1.default('html, body').scrollTop(jquery_1.default("#GameDate").offset().top);
  };

  Game.prototype.displayChoice = function (index, choice) {
    var _this = this;

    console.log("Displaying choice " + index + "."); // Fade the choice in. Trigger the run method again once the fade in has completed.

    jquery_1.default("<p id='choice-" + index + "' class='clickable'>" + choice + "</p>").click(function () {
      return _this.makeChoice(index);
    }).insertBefore("#GameDate").hide().fadeIn(500, this.run.bind(this)); // Force scroll to the bottom of the screen.
...
exports.Game = Game;
},{"jquery":"../node_modules/jquery/dist/jquery.js","./random":"random.ts","./region":"region.ts","./region_events":"region_events.ts","./tribe":"tribe.ts","./tribe_events":"tribe_events.ts"}],"global.ts":[function(require,module,exports) {
"use strict";
...
},{"./game":"game.ts"}],"screens.ts":[function(require,module,exports) {
"use strict";
...
function startPrologue() {
  jquery_1.default("#GameStartScreen").hide();
  jquery_1.default("#GameMainScreen").hide();
  jquery_1.default("#PrologueTitle").hide();
  jquery_1.default("#Prologue1").hide();
  jquery_1.default("#Prologue2").hide();
  jquery_1.default("#Prologue3").hide();
  jquery_1.default("#Prologue4").hide();
  jquery_1.default("#PrologueContinue").hide();
  jquery_1.default("#GamePrologueScreen").show();
  jquery_1.default("#PrologueTitle").fadeIn(2000, function () {
    jquery_1.default("#Prologue1").fadeIn(4000, function () {
      jquery_1.default("#Prologue2").fadeIn(4000, function () {
        jquery_1.default("#Prologue3").fadeIn(4000, function () {
          jquery_1.default("#Prologue4").fadeIn(8000, function () {
            jquery_1.default("#PrologueContinue").fadeIn(2000);
          });
        });
      });
    });
...
exports.startPrologue = startPrologue;

function startMainGame() {
  jquery_1.default("#GamePrologueScreen").hide();
  jquery_1.default("#GameStartScreen").hide();
  jquery_1.default("#GameMainScreen").show();
  jquery_1.default("#GameDate").show();
  global_1.game.start();
}

exports.startMainGame = startMainGame;
},{"jquery":"../node_modules/jquery/dist/jquery.js","./global":"global.ts"}],"startup.ts":[function(require,module,exports) {
"use strict";
...
jquery_1.default(document).ready(function () {
  jquery_1.default("#VersionString").text("Version 0.1 (" + tribe_events_1.TribeEvents.length + " events)");
  jquery_1.default("#StartTitle").hide();
  jquery_1.default("#StartText").hide();
  jquery_1.default("#StartVersionInfo").hide();
  jquery_1.default("#StartTitle").fadeIn(4000, function () {
    jquery_1.default("#StartText").fadeIn(1000, function () {
      jquery_1.default("#StartVersionInfo").fadeIn(500, function () {});
    });
  });
  var width = screen.width * window.devicePixelRatio;

  if (width < 700) {
    jquery_1.default("body").css("border-left", "2px solid");
    jquery_1.default("body").css("border-right", "2px solid");
  }
});
},{"jquery":"../node_modules/jquery/dist/jquery.js","./tribe_events":"tribe_events.ts","./screens":"screens.ts"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","startup.ts"], null)
//# sourceMappingURL=/startup.40672b54.js.map